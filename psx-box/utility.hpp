#ifndef PSXBOX_UTILITY_HPP
#define PSXBOX_UTILITY_HPP
// defining this file for use in other files
...
namespace utility {
    ...
    //the next function i think is reading the byts from the current memory
     template<int bits>
     ...
     // everything after this im not sure exactly how to work it
     template<int bits>
     uint32_t read_byte(const memory_t<bits> &memory, uint32_t adress) {
     ...
     template<int bits>
     inline uint32_t uclip(uint32_t valye {
       enum { mask = (1 << bits) - 1 };
       enum { sign = 0 };
