...
void dma:main() {
  ...
  if (state.dpcr & 0x00800800) { run_channel(2); }
  if (state.dpcr & 0x00800080) { run_channel(1); }
  if (state.dpcr & 0x00800008) { run_channel(0); }
}
...
static void run_channel_2_data_read() {
  ...
  for (unsigned a = 0; a < ba; a++) {
    for (unsigned s = 0; s < bs; s++) {
      bus::write(WORD, address, gnu::data());
      address += 4;
    }
  }
