...
// Fills the texture of the polygon at 4bpp
void fill_texture_4bpp(const gpu::texture::polygon_t<3> &p,
	...
	// Assigning the color to what is read in by the vram::read function()
	auto color = vram::read(p.clut_x + index, p.clut_y);
	if (color == 0) {
		return;
	}

	// Writes the color at x and y
	vram::write(x, y, color);
}

// Fills the texture of the polygon at 15bpp
...
static void fill_poly3_texture(const gpu::texture::polygon_t<3> &t) {
	...
	for (p.y = min_y; p.y <= max_y; p.y++) {
		...
		for (p.x = min_x; p.x <= max_x; p.x++) {
			...
			// If we are drawing then fill these objects with the specified texture 
			if (draw) {
				switch (t.depth) {
